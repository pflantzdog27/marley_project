...
window.onload = function() {
    ...
    var marleyLogoWidth = $('#marley-logo').width();
    var left = (($(window).width() - $('.container').width()) / 2) + (marleyLogoWidth/2);
    particles = new ParticleCanvas(canvas, {x: left, y: $(window).height()});
    particles.start();
};
function extend() {
...
var ParticleGenerator = function(options) {
    ...
    var options = extend({
        ...
        opacity:.7,
        onDraw: function(p) {
            p.opacity = 0.051 - (p.age / p.life) * 0.025;
        }
    }, options);
